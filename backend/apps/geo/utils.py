...
class ProgressTracker:
    ...
    def to_dict(self):
        ...
        return {
            ...
            "result_geojson": self.result_geojson,
            "error": self.error,
        }
...
def classify_buildings(input_gdf, output_dir, zoom=19, session_id=None):
    ...
    try:
        result_gdf = _classify_buildings_inner(
            session_id, input_gdf, output_dir, zoom, progress
        )
        result_gdf["geometry"] = result_gdf["geometry"].apply(to_2d_geom)
        progress.result_geojson = json.loads(result_gdf.to_json())
        progress.set_phase("complete", "Classification finished")
    except Exception as exc:
        logger.exception("classify_buildings failed: %s", exc)
        progress.error = str(exc)
        progress.set_phase("complete", f"Classification failed: {exc}")
    finally:
            ...
            for bld_idx, row in bld_list:
                ...
                if sam_poly is None:
                    ...
                    if iou >= iou_unchanged:  # iou >= 0.35:
                        ...
                        iou < iou_unchanged and iou >= iou_modified_low
                    ):  # 0.10 <= iou < 0.35
                        status = "modified"
                    else:  # iou < 0.10
                        try:
                            ...
                            logger.warning(
                                f"Overlap ratio computation failed for building {bld_idx}: {e}"
                            )
                            overlap_ratio = 0.0
